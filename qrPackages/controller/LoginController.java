			...
			if (USER_ID == null || USER_ID == "")
			{
				...
				else
				{
					...
					if(member.get("level") != null)
					{
						...
						if((level & 0x01) > 0){member.put("m_qr", "1");}else{member.put("m_qr", "0");}
						if((level & 0x02) > 0){member.put("m_stats", "1");}else{member.put("m_stats", "0");}
						if((level & 0x04) > 0){member.put("m_member", "1");}else{member.put("m_member", "0");}
					}
					request.getSession().setAttribute("USER_ID", user_id);
					request.getSession().setAttribute("USER_NICKNAME", nickname);
					request.getSession().setAttribute("USER_INFO", member);
					mav.setViewName("redirect:/main/frame");
				}
			}
